...
const poke = parseNotice({
  ...
  user_id: 10001, target_id: 3000000001, group_id: 100000001, self_id: 3000000001,
})
check('poke 解析 noticeType', poke.noticeType === 'notify' && poke.subType === 'poke', JSON.stringify(poke))
check('poke 解析 user/target', poke.userId === 10001 && poke.targetId === 3000000001)
check('poke 解析 groupId', poke.groupId === 100000001)

// 私聊戳一戳（无 group_id）
const poke2 = parseNotice({ post_type: 'notice', notice_type: 'notify', sub_type: 'poke', user_id: 10001, target_id: 3000000001 })
...
const join = parseNotice({
  ...
  group_id: 100000001, user_id: 20002, operator_id: 10001, self_id: 3000000001,
})
...
// 退群
const leave = parseNotice({ post_type: 'notice', notice_type: 'group_decrease', group_id: 100000001, user_id: 20002 })
check('退群 noticeType', leave.noticeType === 'group_decrease' && leave.groupId === 100000001)
