...
Only the two most recent major lines are listed here (one bullet per line; per-release detail lives in [CHANGELOG.md](CHANGELOG.md)):

- **v0.5 line (v0.5.0 → v0.5.7, current)** — the theme evolved from "See it, find it" through "Unattended", "One tap", "Group ops toolbox" and "Console visibility" to real-machine usability fixes and an audit. **v0.5.0** merged-forward expansion plus `/成员`, `/群信息`, `/好友`, `/文件`, `/取`, `/相册`, `/ocr` and a per-day archive with `/找`; **v0.5.1** eight defects fixed after an adversarial review; **v0.5.2** the unattended trio (inbound webhook with token/HMAC auth and rate limits, scheduled RSS/weather/MC broadcasts, auto-heal that only ever starts a process); **v0.5.3** light interactions (active poke, poke back, private-chat typing status, emoji-reaction statistics with `/赞榜` and `/谁赞了`, profile likes, mark-as-read) — **the real-device probe proved this NapCat build cannot send inline buttons**, so no button panel was built; **v0.5.4** the group ops toolbox (native check-in, @all quota, mute list, native batch kick with two-step confirm and batching, group todos, file organisation, album upload, group profile, join/speak policy, weekly ops report); **v0.5.5** five console panels (P50/P95 performance, scheduled jobs, group config, replay diff, a 17-scenario injection library); **v0.5.6** real-machine usability fixes plus a full audit (elevated NapCat launch, login QR panel, token guidance, and the **severe fix for killing QQ by image name, which would have killed your own client**). The red lines held throughout: trace plus a true reason, whitespace-separated commands, zero outbound frames on injected turns, and every write through the ActionGate. **55 suites / 3434 assertions green**.
