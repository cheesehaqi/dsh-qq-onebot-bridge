...
只列最近两个大版本（每个大版本一条线；逐版明细见 [CHANGELOG.md](CHANGELOG.md)）：

- **v0.5 线（v0.5.0 → v0.5.7，当前）** — 主题一路演进：「看得见 · 找得回」→「无人值守」→「点一下就完事」→「群运营工具箱」→「控制台看得见」→ 真机可用性修复。**v0.5.0** 合并转发展开 + `/成员` `/群信息` `/好友` `/文件` `/取` `/相册` `/ocr` + 按天归档与 `/找`；**v0.5.1** 发布后对抗审查修掉 8 个缺陷；**v0.5.2** 无人值守三件套（入站 webhook 带 token/HMAC 鉴权与限频、定时播报 RSS/天气/MC、掉线自愈只启动不杀进程）；**v0.5.3** 轻互动（主动戳一戳 / 被戳回戳 / 私聊正在输入 / 表情回应统计 `/赞榜` `/谁赞了` / 点赞 / 标记已读）——**真机探针确认该 NapCat 构建发不了内联按钮**，所以没做按钮面板；**v0.5.4** 群运营工具箱（原生群签到 / @全体余量 / 禁言名单 / 原生批量踢（两步确认 + 分批）/ 群待办 / 文件整理 / 相册上传 / 群资料 / 入群与发言策略 / 运营周报）；**v0.5.5** 控制台五件套（性能面板 P50·P95 / 定时任务面板 / 群配置页 / 回放 diff / 注入场景库 17 个）；**v0.5.6** 真机可用性修复 + 完整审计（提权启动 NapCat、登录二维码面板、token 引导，并**修掉"按镜像名杀 QQ 会误杀你自己 QQ"的严重缺陷**）。红线贯穿全线：trace + 真实 reason、命令空白分隔、注入回合 0 出站、写操作过 ActionGate。全量 **55 套 / 3434 断言全绿**。
- **v0.4 线（v0.4.0 → v0.4.1）** — 主题「一切皆可调试 / Everything Debuggable」，6 条硬约束：①无静默分支必带 reason ②一条消息一个 traceId 贯穿 ③可回放（dry-run）④可体检 ⑤可导出诊断包 ⑥可注入假事件——全部落地并由控制台「硬约束验收台」实时验收。**v0.4.0** 带来 traceId 结构化事件与 SSE 实时事件流、一键体检、脱敏诊断包导出、录制 / 离线回放 / 事件注入（一次注入 = 一个独立回合且整回合不碰 QQ），以及自带独立控制台（`control/`，8799）；**v0.4.1** 修依赖解析（改用 `@deepseek-ai/schemastery`，社区 issue #1）。
