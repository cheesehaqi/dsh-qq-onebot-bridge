...
- **v0.2.7** — 调试日志轮转、图片保留期清理（imageRetentionDays）、提醒解析单元测试
- **v0.2.6** — 可配置识图方式（`visionMode: tool/native` 原生多模态附件 + `visionToolName`）
- **v0.2.5** — 定时提醒（群聊@触发/私聊关键词、跨重启保留、`/reminders`）
- **v0.2.4** — 每会话持久化记忆（重启不失忆）
- **v0.2.3** — 安全默认值：白名单为空 = 拒绝

完整历史见 [CHANGELOG.md](CHANGELOG.md)（更早版本见 git 提交历史）。
