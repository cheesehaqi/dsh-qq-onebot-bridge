...
export function planTodo({ groupId, kind = 'set', messageId = '', messageSeq = '' } = {}) {
  // 必须用 hasOwn：`TODO_ACTIONS['constructor']` 会顺着原型链拿到函数（审查提的 O5）。
  const action = Object.hasOwn(TODO_ACTIONS, kind) ? TODO_ACTIONS[kind] : ''
...
export function planFileOp({ groupId, kind, fileId = '', currentParent = '', targetParent = '', newName = '', folderName = '' } = {}) {
  const action = Object.hasOwn(FILE_ACTIONS, kind) ? FILE_ACTIONS[kind] : ''
...
export function planProfileChange({ groupId, kind = 'name', value = '' } = {}) {
  const action = Object.hasOwn(PROFILE_ACTIONS, kind) ? PROFILE_ACTIONS[kind] : ''
