...
export const FRAME_FIELDS = {
  message: ['messageType', 'userId', 'groupId', 'text', 'atMe', 'ats', 'reply', 'records', 'images', 'files', 'forwards', 'forwardText', 'messageId', 'senderName'],
...
export function serializeFrame(kind, frame, { redact = false, now = Date.now() } = {}) {
  ...
  for (const field of fields) {
    ...
    else if (field === 'forwardText' && typeof value === 'string') body[field] = value.slice(0, 4000)
 ...
 *         forwards: [转发卡片 id], forwardText: 预先展开好的转发正文 }
 */
export function expandInjection(spec = {}, { botQq = 0, now = Date.now() } = {}) {
  ...
  // 转发卡片在 dry-run（回放/注入）下取不到 QQ 的真实内容，所以允许注入时直接带一份展开好的正文，
  // 这样「⑥可注入」能确定性地覆盖转发路径；真实运行仍以 get_forward_msg 的结果为准。
  const forwards = Array.isArray(spec.forwards)
    ? spec.forwards.map((id) => ({ id: String(id) })).filter((item) => item.id !== '')
    : []
  const forwardText = String(spec.forwardText ?? '').trim()
  if (!text && images.length === 0 && records.length === 0 && files.length === 0 && forwards.length === 0) {
    throw new Error('消息注入至少要有文本、图片/语音/文件或转发卡片（forwards）')
  }
  ...
  if (forwardText) frame.forwardText = forwardText.slice(0, 4000)
  return { kind, frame }
}
