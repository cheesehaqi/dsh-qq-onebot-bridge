...
export const DEFAULT_ACTION_LIMITS = {
  ...
  set_group_leave: { perMinute: 1, perDay: 3 },
  // v0.5.3 互动：戳一戳 / 正在输入 / 标记已读同样是「会真的动到 QQ 账号」的写操作。
  group_poke: { perMinute: 3, perDay: 60 },
  friend_poke: { perMinute: 3, perDay: 60 },
  set_input_status: { perMinute: 20, perDay: 500 },
  mark_group_msg_as_read: { perMinute: 12, perDay: 1000 },
  mark_private_msg_as_read: { perMinute: 12, perDay: 1000 },
}
...
export class ActionGate {
  ...
  constructor({ auditFile = '', limits = {}, perMinute = 0, perDay = 0, now = () => Date.now() } = {}) {
    ...
    this.denied = 0
    // 注入/回放（dry-run）期间由桥置为 true：仍然做 check（好让"会不会被限流"照样报出来），
    // 但**不 commit**——否则干跑一次就会吃掉真实的每分钟/每日配额，还会在 qq-actions.log 里
    // 留下一条"从未真正发生过"的写操作审计。
    this.dryRun = false
  }
   ...
   * Returns `{ ok, skipped, reason, dryRun, value }`; `fn` errors propagate.
   *
   * `dryRun` 为 true 时（注入/回放的干跑）只判不记账：调用方据此知道"这次没有真的执行"。
   */
  async run(action, key, detail, fn) {
    const verdict = this.check(action, key)
    if (!verdict.ok) {
      this.denied += 1
      if (this.auditFile && this.dryRun !== true) {
        appendCappedLine(this.auditFile, `${new Date(this.now()).toISOString()} DENIED ${action} ${key} reason=${verdict.reason}`)
      }
      return { ok: false, skipped: true, reason: verdict.reason, dryRun: this.dryRun === true, value: undefined }
    }
    if (this.dryRun !== true) this.commit(action, key, detail)
    const value = await fn()
    return { ok: true, skipped: false, reason: '', dryRun: this.dryRun === true, value }
  }
