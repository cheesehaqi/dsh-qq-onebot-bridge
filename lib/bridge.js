...
import { appendFileSync, mkdirSync, writeFileSync, existsSync, readFileSync, unlinkSync, readdirSync, statSync, rmSync } from 'node:fs'
...
export class QQBridge {
  ...
  async #onQqMessageInner(message) {
    ...
    if (this.config.verifyEnabled && message.messageType === 'private' && await this.#handleJoinAnswer(message, text)) return
    // 防撤回缓存：每条入站消息都留一份（含图片 URL），撤回时补发。
    if (this.config.antiRecallEnabled) this.#rememberForRecall(message, text)
    // 敏感词 / 刷屏：命中即拦下，不再进入模型。
    if ((this.config.filterEnabled || this.config.floodEnabled) && await this.#handleContentFilter(message, text)) return
    ...
    if (this.config.gameEnabled && await this.#handleActiveGame(message, text)) return
    // ---- 关键词问答库（免模型、免 @；默认关闭）----
    if (this.config.keywordEnabled && await this.#handleKeyword(message, text)) return
    ...
    if (this.config.fortuneEnabled && await this.#handleFortune(message, text)) return
    if (this.config.diceEnabled && await this.#handleDice(message, text)) return
    if (this.config.pointsEnabled && await this.#handlePointsCommand(message, text)) return
    if (this.config.statsEnabled && await this.#handleStatsCommand(message, text)) return
    if (this.config.mcStatusEnabled !== false && await this.#handleMcStatus(message, text)) return
    if (this.config.gameEnabled && await this.#handleGameCommand(message, text)) return
    if (this.config.dailyReportEnabled && await this.#handleDailyReportCommand(message, text)) return
    if (this.config.groupReadEnabled !== false && await this.#handleGroupRead(message, text)) return
  ...
  async #onRequest(request) {
    ...
    try {
      ...
      if (request.requestType === 'group' && request.groupId && !this.config.allowGroups.includes(request.groupId)) return
  ...
  async #createSession(message, key) {
    ...
    const entry = this.#registerSession(key, route, handle)
  ...
  async #resumeSession(message, key, sessionId) {
    ...
    return this.#registerSession(key, route, handle)
  }

  #registerSession(key, route, handle) {
  ...
  async #replyTo(route, text, { forceForward = false } = {}) {
