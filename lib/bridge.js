...
export class QQBridge {
  ...
  async #handleInjection(kind, frame) {
    ...
    try {
      ...
      if (dryRun) {
        ...
        this.trace?.event({
          id: '', level: 'info', module: 'bridge', stage: 'inject', ok: true, chatKey,
          reason: `注入完成：${calls.length} 个出站调用被拦截（dry-run，未发送）`,
  ...
  async #onQqMessage(message) {
    ...
    if (this.config.recordInbound !== false && message.__injected !== true) this.inbox.record('message', message)
    try {
  ...
  async #onNotice(notice) {
    if (this.stopped) return
    if (this.config.recordInbound !== false && notice.__injected !== true) this.inbox.record('notice', notice)
  ...
  async #onRequest(request) {
    if (this.stopped) return
    if (this.config.recordInbound !== false && request.__injected !== true) this.inbox.record('request', request)
  ...
  #registerSession(key, route, handle, { resumed = false } = {}) {
    const entry = { key, route, handle, agent: handle.agent, sessionId: String(handle.agent.id), resumed, lastTraceId: '', lastTurnAt: 0, turnStartedAt: 0 }
  ...
  #onSessionEvent(session, event) {
    ...
    if (event.type === 'turn/start') {
      ...
      if (entry) entry.turnStartedAt = Date.now()
      this.trace?.event({
