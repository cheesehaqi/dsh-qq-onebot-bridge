...
export class QQBridge {
    ...
    if (hasForwards && this.config.forwardExpandEnabled !== false) {
      try {
        ...
        if (expanded.text) {
          ...
          mark('forward', true, `转发内容已展开（${expanded.used}/${expanded.total} 条${expanded.truncated ? '，已截断' : ''}，来源 ${expanded.source}）`)
        } else {
  ...
  async #queryGroupMembers(groupId, query) {
    ...
    if (!target && query.name) {
      ...
      const nameOf = (item) => String(item?.card ?? item?.nickname ?? '').toLowerCase()
  ...
  async #handleOcr(message, text) {
    ...
    // ocr_image 的参数只有 {image}，不带 group_id/user_id，**落不进作用域 dry-run 的拦截条件**，
    // 所以这里必须像 #expandForwards 一样显式判离线：回放/注入一律不碰 QQ。
  ...
  async #handleGroupFiles(message, text) {
    ...
    try {
      ...
      try {
        ...
        await this.#safeReply(message, `✅ 已下载到本机：${target}（私聊发送失败：${error.message.slice(0, 60)}）`)
      }
      return true
    } catch (error) {
  ...
  async #expandForwards(message) {
    ...
    const ids = (Array.isArray(message.forwards) ? message.forwards : [])
      .map((item) => (item && item.id !== undefined && item.id !== null ? String(item.id) : ''))
      .filter((id) => id !== '')
      .slice(0, 3)
    if (ids.length === 0) {
      return { text: '', used: 0, total: 0, truncated: false, source: 'none', reason: '转发卡片没有 id，无法展开' }
    }
    ...
    if (parts.length === 0) {
      return {
        text: '', used: 0, total, truncated, source: dryRun ? 'dry-run' : 'empty',
        reason: dryRun
    ...
    return { text: parts.join('\n'), used, total, truncated, source: 'api' }
  }
