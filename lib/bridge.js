...
export class QQBridge {
  ...
  async #handleInjection(kind, frame) {
    ...
    const previous = dryRun ? this.server.setDryRun(true) : this.server.dryRun
    this.injecting = true
    ...
    try {
      ...
      if (dryRun) {
        ...
        this.trace?.event({
          id: '', level: 'info', module: 'bridge', stage: 'inject', ok: true, chatKey,
          reason: `注入完成：同步阶段 ${calls.length} 个出站调用被拦截；agent 回合的回复也会被拦截（dry-run，均未发送）`,
          data: { wouldSend: calls.map((call) => ({ action: call.action, text: summarizeSegments(call.params?.message) })) },
        })
      }
    } finally {
      if (dryRun) this.server.setDryRun(previous)
      this.injecting = false
      this.#writeRuntimeSnapshot(true)
    }
  }
  ...
  async #onQqMessageInner(message) {
    ...
    try {
      ...
      // 注入帧走到了 agent：异步回合的回复不会被 #handleInjection 的 dry-run 窗口覆盖，
      // 这里记一笔，由 #onSessionEvent 拦下（injectDryRun=false 时才是真的允许发出去）。
      if (message.__injected === true && this.config.injectDryRun !== false) {
        entry.injectPending = (entry.injectPending ?? 0) + 1
        mark('inject', true, '注入帧已转交 agent：本回合的回复会被 dry-run 拦下')
      }
  ...
  async #ensureSession(message) {
    ...
    if (existing !== undefined) {
      // 真实消息立刻解除"注入回合"标记：注入绝不能让真人的回复也被拦下
      if (message.__injected !== true) existing.injectPending = 0
      return existing
    }
  ...
  #registerSession(key, route, handle, { resumed = false } = {}) {
    const entry = {
      ...
      // 注入回合记账：injectPending>0 表示"等着被拦下的注入回合"，
      // turnHadInjection/turnSuppressedReply 是这一回合的临时状态（见 #onSessionEvent）
      injectPending: 0, turnHadInjection: false, turnSuppressedReply: false,
    }
  ...
  #onSessionEvent(session, event) {
    ...
    if (event.type === 'turn/start') {
      ...
      if (entry) {
        ...
        entry.turnHadInjection = (entry.injectPending ?? 0) > 0
    ...
    if (event.type === 'turn/end') {
      ...
      if (entry) {
        // 注入回合结束：无论模型有没有回话，都要把这次记账清掉，避免"漏拦"下一回合
        if (entry.turnHadInjection) entry.injectPending = Math.max(0, (entry.injectPending ?? 1) - 1)
        entry.turnHadInjection = false
        entry.turnSuppressedReply = false
      }
      this.trace?.event({
    ...
    if ((entry.injectPending ?? 0) > 0 && entry.turnSuppressedReply !== true) {
