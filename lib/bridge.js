...
export class QQBridge {
  ...
  async #sendTts(route, text) {
    try {
      ...
      const audio = await synthesizeTts(this.config, spoken)
      if (!audio || audio.length === 0) throw new Error('TTS 返回空音频')
      mkdirSync(this.ttsDir, { recursive: true })
      const file = join(this.ttsDir, `tts-${Date.now().toString(36)}.mp3`)
      ...
      debugLog(`tts sent (${audio.length} bytes)`)
    } catch (error) {
    ...
    for (const f of (message.files ?? []).slice(0, 4)) {
      ...
      if (url && /^https?:\/\//.test(url)) {
        try {
          ...
          const name = f.name ? basename(String(f.name)) : `file-${Date.now().toString(36)}`
          const file = join(this.filesDir, name)
  ...
  #onSessionEvent(session, event) {
    ...
    if (this.config.ttsEnabled) {
      void this.#sendTts(entry.route, text)
    }
  }
