...
export class QQBridge {
  ...
  async #queryGroupMembers(groupId, query) {
    ...
    if (query.kind === 'list') {
      const list = membersOf(await this.server.getGroupMemberList(socket, groupId, true))
      const formatted = formatMemberList(list, { limit: this.config.memberListLimit, selfId })
      return { ok: true, text: formatted.text }
    }
    let target = Number(query.userId) || 0
    if (!target && query.name) {
      const list = membersOf(await this.server.getGroupMemberList(socket, groupId, false))
