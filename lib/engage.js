...
export function planSendLike({ userId, times = 1, max = MAX_LIKE_TIMES } = {}) {
  ...
  const limit = Math.max(1, Math.min(MAX_LIKE_TIMES, Number(max) || MAX_LIKE_TIMES))
  const wanted = Number(times)
  const wantedOk = Number.isFinite(wanted) ? Math.trunc(wanted) : 1
