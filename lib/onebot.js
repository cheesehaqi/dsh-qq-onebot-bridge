...
export function parseMessage(rawMessage, botQq) {
  ...
  return { text: text.replace(/\s+/g, ' ').trim(), ats, records, images, files, reply }
}
...
export class OneBotServer extends EventEmitter {
  ...
  constructor(config, logger) {
    ...
    this.logger = logger
    /**
     * Dry-run mode (v0.4 debugging): when true, EVERY action call is recorded and
     * stubbed instead of being sent — the injector uses it so a synthetic frame can
     * run through the real pipeline with zero QQ side effects.
     */
    this.dryRun = false
    this.dryRunCalls = []
  }

  /** Turn dry-run on/off; returns the previous value. */
  setDryRun(enabled) {
    const previous = this.dryRun
  ...
  #onFrame(socket, data) {
    ...
    if (frame.post_type === 'message' && frame.message_type) {
      ...
      if ((parsed.text === '' && parsed.records.length === 0 && parsed.images.length === 0 && parsed.files.length === 0) || !Number.isFinite(userId)) return
  ...
  #call(socket, action, params) {
    // 调试用 dry-run：只记录，不发送（注入器走这条路，绝不会有 QQ 副作用）。
    if (this.dryRun) {
